...
// Tiny canvas chart component (no external libs)
...
<template>
  <div class="dashboard">
    ...
    <main class="two-col">
      <!-- Left: scrollable list and controls -->
          ...
          >
            ...
            <div class="card-actions row">
              ...
              <input
                type="email"
                :value="draftFor(it)"
                placeholder="zuständig@example.com"
                @input="(e:any)=>{ drafts[it.id] = e.target.value }"
                @keydown.enter.prevent="saveEmail(it)"
              />
...
.card{
  ...
  box-shadow:0 2px 10px rgba(21,101,192,.06);display:flex;flex-direction:column;gap:8px
}
.card.list-item{ }
...
.switch-slider{
  ...
  background:#dce6ff;border:1px solid var(--border);transition:.2s
}
.switch-slider::after{
  content:'';position:absolute;width:20px;height:20px;top:1px;left:1px;border-radius:50%;
